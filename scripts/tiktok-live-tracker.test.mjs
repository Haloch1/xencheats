...
import { tikTokLiveHandle, isTikTokShareLink, resolveTikTokLiveHandle, parseTikTokLiveResponse, readTikTokLive, readTikTokLiveFromPublicPage, liveDurationWindow, formatLiveDuration, sendTikTokLiveReport } from "../lib/tiktok-live-tracker.mjs";
...
let providerCalls = 0;
const publicFirst = await readTikTokLive("creator", "provider-key", async (url) => {
  providerCalls += 1;
  if (url.includes("api.scrapecreators.com")) throw new Error("paid provider should not be called");
  return new Response('<script id="SIGI_STATE" type="application/json">{"LiveRoom":{"liveRoomUserInfo":{"user":{"uniqueId":"creator","status":4},"liveRoom":{"status":4}}}}</script>', { status: 200 });
});
assert.equal(publicFirst.isLive, false);
assert.equal(providerCalls, 1);

const fallback = await readTikTokLive("creator", "provider-key", async (url) => {
  providerCalls += 1;
  if (url.includes("api.scrapecreators.com")) return new Response(JSON.stringify({
    success: true,
    is_live: true,
    liveRoomUserInfo: { uniqueId: "creator", roomId: "123" },
    liveRoom: { startTime: 1790164800 },
    liveRoomStats: { userCount: 9 },
  }), { status: 200, headers: { "content-type": "application/json" } });
  return new Response("blocked", { status: 403 });
});
assert.equal(fallback.isLive, true);
assert.equal(fallback.viewerCount, 9);

...
function mediaImageMarkup(item) {
  const source = mediaArtwork(item);
  const fallback = mediaCategoryArtwork[item?.category] || mediaFallbackArtwork;
  return `<img src="${esc(source)}" data-media-artwork alt="" loading="lazy" data-fallback-src="${esc(fallback)}" />`;
}
...
productList?.addEventListener("error", (event) => {
  const image = event.target.closest("[data-media-artwork]");
  if (!image || image.dataset.fallbackUsed === "true") return;
  image.dataset.fallbackUsed = "true";
  image.src = image.dataset.fallbackSrc || mediaFallbackArtwork;
}, true);

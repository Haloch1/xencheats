...
The optional Cheats.Love browser workflow (`npm run finance:workflow:simulate`) reuses an authenticated Playwright session, uses accessible selectors, reads the top-up/USDC invoice details, and stops before payment submission. CAPTCHA, 2FA, and security challenges always stop the workflow. Set `CHEATSLOVE_APP_URL` and `CHEATSLOVE_STORAGE_STATE` only when a supervised supplier session is available. Live execution is disabled by `FINANCE_LIVE_EXECUTION_ENABLED=false`.

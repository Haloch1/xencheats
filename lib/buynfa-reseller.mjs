...
function collectCatalogProducts(payload) {
  ...
  if (Array.isArray(catalog)) {
    const categories = catalog.filter((category) => Array.isArray(category?.products ?? category?.items));
    if (categories.length) {
      return categories.flatMap((category) => array(category.products ?? category.items)
        .map((product) => ({ product, parentCategory: category })));
    }
    return catalog.map((product) => ({ product, parentCategory: null }));
  }
  const source = catalog && typeof catalog === "object" ? catalog : root;

  const topLevelCategories = array(source?.categories ?? source?.data?.categories);
  const entries = [];
  ...
  const topLevelProducts = array(source?.products ?? source?.items ?? source?.data?.products ?? source?.data?.items);
  if (topLevelProducts.length) return topLevelProducts.map((product) => ({ product, parentCategory: null }));
  const fields = source && typeof source === "object" ? Object.keys(source).slice(0, 20).join(", ") : typeof source;

...
export async function readTikTokLive(handle, apiKey, fetchImpl = fetch) {
  ...
  try {
    // Prefer TikTok's public LIVE page so normal checks do not consume paid
    // provider credits. The paid endpoint remains a reliability fallback.
    return await readTikTokLiveFromPublicPage(handle, fetchImpl);
  } catch (error) {
    publicPageError = error;
  }
  if (!apiKey) throw publicPageError || new Error("TikTok LIVE public page unavailable");
  ...
  if (!response.ok) throw publicPageError || new Error(`TikTok LIVE provider HTTP ${response.status}`);

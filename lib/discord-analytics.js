...
export function createDiscordAnalytics({ supabase, guildId, enabled = true, presenceEnabled = false, snapshotMinutes = 5, logger = console }) {
  ...
  function matchInviteForJoin() {
    ...
    for (const [code, current] of inviteUses.entries()) {
      if (Number(current.lastDelta) > 0 && Date.now() - Number(current.deltaAt || 0) <= 30_000) changed.push(current);
    }
  ...
  async function refreshInvites(guild = activeGuild) {
    ...
    for (const invite of fetched.values()) {
      ...
      const delta = Math.max(0, Number(row.uses) - Number(prior?.uses ?? row.uses));
      inviteUses.set(invite.code, { ...row, lastDelta: delta, deltaAt: delta ? Date.now() : null });
      rows.push(row);
  ...
  async function recordVoiceState(oldState, newState) {
    ...
    await safely("record voice state", async () => {
      ...
      if (oldChannelId && oldChannelId !== newChannelId && open) {
        const closeResult = await supabase.from("discord_analytics_voice_sessions").update({ ended_at: asIso(at) })
  ...
  async function start(guild) {
    ...
    void (async () => {
      await resumeBackfillJobs();
      await safely("queue startup imports", async () => {
      const [{ data: recentJobs, error: jobsError }, memberCountResult] = await Promise.all([
        supabase.from("discord_analytics_backfill_jobs").select("kind,status,processed_count,created_at,updated_at").eq("guild_id", String(guildId)).order("created_at", { ascending: false }).limit(50),
        supabase.from("discord_analytics_members").select("user_id", { count: "exact", head: true }).eq("guild_id", String(guildId)),
      ]);
      if (jobsError) throw jobsError;
      const knownJobs = recentJobs || [];
      const activeOrRecentVoiceImport = knownJobs.some((job) => job.kind === "voice_reconcile" && (job.status === "queued" || job.status === "running" || Date.now() - new Date(job.updated_at || job.created_at).getTime() < 6 * 60 * 60_000));
      if (!activeOrRecentVoiceImport) void createBackfillJob("voice_reconcile").catch((error) => fail("voice reconcile", error));
      // A new installation starts with no roster. Load it once automatically;
      // later restarts use the saved job cursor and do not create duplicates.
      if (!Number(memberCountResult.count) && !knownJobs.some((job) => job.kind === "members" && ["queued", "running"].includes(job.status))) void createBackfillJob("members").catch((error) => fail("member import", error));
      });
    })().catch((error) => fail("resume backfills", error));

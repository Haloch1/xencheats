...
export function getCommonSupportReply(message, history = [], hasAttachment = false) {
  ...
  if (/\b(missing|didn.t get|did not get|where.*key|key.*missing|unfulfilled|not fulfilled)\b/i.test(context)) {
    return "Send the Order ID shown on your Account page and I can check its real delivery status. Do not post your license key or password.";
  }
  if (hasAttachment) {
...
export function classifyTranscriptEvidence(messages = []) {
  ...
  if (explicitResolution && writtenFix && issue) return { level: "verified", issue, writtenFix };
